    ...
    return [
        Tool(
            ...
            description = (
                ...
                "diagrams (architecture, data flow, component map, ER, sequence, "
                "deployment) plus a plain-text overview in a single call.  "
                "Accepts a GitHub URL or 'owner/repo' slug."
            ),
        ...
        Tool(
            ...
            description = (
                "Generate a layered system architecture diagram (Mermaid graph TB) "
                "showing client, backend, data, and infrastructure layers with "
                "auto-detected services (auth, cache, message queue, etc.)."
            ),
        ...
        Tool(
            ...
            description = (
                "Generate a data flow diagram (Mermaid flowchart LR) tracing a "
                "request from the user through the API gateway, optional auth "
                "middleware, business logic, cache, and database, with detected "
                "API routes annotated."
            ),
            inputSchema = _REPO_SCHEMA,
        ),
        Tool(
            ...
            description = (
                "Generate an Entity-Relationship diagram (Mermaid erDiagram) by "
                "extracting ORM model definitions and their FK/relation fields from "
                "Django models.py, Prisma schema.prisma, TypeORM @Entity classes, "
                "SQLAlchemy declarative models, Sequelize define() calls, and Rails "
        ...
        Tool(
            ...
            description = (
                "Generate a numbered sequence diagram (Mermaid sequenceDiagram) "
                "showing the full request lifecycle including optional auth token "
                "validation, cache lookup with hit/miss branches, DB queries, and "
        ...
        Tool(
            ...
            description = (
                "Generate a module dependency diagram (Mermaid graph LR) built "
                "from static import analysis across Python, TypeScript, JavaScript, "
                "Go, Java, Rust, and Ruby source files."
            ),
            inputSchema = _REPO_SCHEMA,
        ),
        Tool(
            ...
            description = (
                "Generate a deployment topology diagram (Mermaid graph TB) by "
                "detecting Docker, Kubernetes, Helm, Terraform, and CI/CD "
                "configuration files, including external service integrations."
            ),
            inputSchema = _REPO_SCHEMA,
        ),
        Tool(
            ...
            description = (
                ...
                "Useful as a quick first look before generating diagrams."
            ),
            inputSchema = _REPO_SCHEMA,
        ),
    ]
...
async def call_tool(name: str, arguments: dict) -> list[TextContent]:
    ...
    diagram_fn = {
        "generate_architecture_diagram": lambda r: diagrams.architecture(r),
        "generate_data_flow_diagram":    lambda r: diagrams.data_flow(r),
        "generate_er_diagram":           lambda r: diagrams.er_diagram(r.models),
        "generate_sequence_diagram":     lambda r: diagrams.sequence(r),
        "generate_component_diagram":    lambda r: diagrams.component(r),
        "generate_deployment_diagram":   lambda r: diagrams.deployment(r),
    }.get(name)

    if diagram_fn:
        heading = name.replace("generate_", "").replace("_", " ").title()
        return [TextContent(type="text", text=
            f"## {heading} — `{result.owner}/{result.repo}`\n\n"
            + diagram_fn(result)
        )]

    if name == "analyze_repository":
        return [TextContent(type="text", text=_full_analysis(result))]
